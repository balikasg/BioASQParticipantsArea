...
The package contains two folders "flat/" and "hierarchical/" corresponding to the flat and hierarchical measures that will be used during the evaluation of the challenge Task1a. Additionally, a folder "mesh/" contains the hierarchy in parent-child relations both in the original format (mesh_hierarchy.txt) and in mapped format using integers (mesh_hiearchy_int.txt) as well as the corresponding mapping (mapping.txt).
...
java -Xmx10G -cp $CLASSPATH:./flat/BioASQEvaluation.jar converters.MapMeshResults mesh/mapping.txt system_A_results.txt system_A_results_mapped.txt
...
java -Xmx10G -cp $CLASSPATH:./flat/BioASQEvaluation.jar evaluation.Evaluator true_labels_mapped.txt system_A_results_mapped.txt
