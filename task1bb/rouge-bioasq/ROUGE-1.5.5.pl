...
if(defined($doc)) {
  ...
  foreach $peerID (@allPeerIDs) {
	...
	if($n==2){ # change by Ioannis
	&printResultsBioASQ($peerID,\%ROUGEAverages,\%ROUGEScores,"ROUGE-$n",$opt_c,$opt_t,$opt_d);
	}
      }
    }
	...
	&computeROUGEX("S",\%ROUGEScores,$e,$ROUGEEvals{$e},$peerID,\%ROUGEParam);
      ...
      if($skipDistance>=0) {
	if(defined($opt_u)) {
	  &printResultsBioASQ($peerID,\%ROUGEAverages,\%ROUGEScores,"ROUGE-SU$skipDistance",$opt_c,$opt_t,$opt_d);
	}
	elsif(defined($opt_U)) {
...
# recall, precision, f-measure
    print "$ROUGEAverages->{'AvgR'} $ROUGEAverages->{'AvgP'} $ROUGEAverages->{'AvgF'} ";
  }
}
