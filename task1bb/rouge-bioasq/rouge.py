...
def rouge(gold_results, system_results):
	...
	os.environ['PYTHONPATH']='/home/bioasq/packages/lib/python2.7/site-packages/'
		...
		with open('/home/bioasq/public_html/webexample/task1bb/rouge-bioasq/log4444', 'w') as out:
	                json.dump(["%s"%question['id']], out, indent=2)
		string_golden='<html>\n<head>\n<title>golden</title>\n</head>\n<body bgcolor="white">\n<a name="1">[1]</a> <a href="#1" id=1>{0}</body>\n</html>'.format(gquestion['ideal_answer'].encode('ascii','ignore'))
		with open('/home/bioasq/public_html/webexample/task1bb/rouge-bioasq/log4444', 'w') as out:
                        json.dump([string_golden], out, indent=2)
